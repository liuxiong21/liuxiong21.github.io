...
## 主要使用场景
大家考虑下在Netty这种系统里面会有大量的并发连接，而每一个并发连接都需要创建一个Channel,每一个Channel对应一个Input ChannelPipeline，而一个ChannelPipeline对应由多个ChannelHandlerContext组成的链。这个时候就会有大量的ChannelHandlerContext实例存在，并且是长时间存活着的对象。这个时候使用AtomicXXFieldUpdater就比AtomicXX合适了。为什么？
因为一个int字段只占4个Byte，而一个AtomicInteger需要占用除对象引用的4个Byte外，AtomicInteger内部保存Integer字段的4个Byte，翻了一倍。
